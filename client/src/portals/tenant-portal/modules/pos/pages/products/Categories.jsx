...
export default function Categories() {
  ...
  const { products, outlets, reload: reloadRef } = usePosReference();
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const data = await fetchAllTableRows("/pos/inventory/categories", authFetch);
      setRows(data);
    } catch {
      setRows([]);
    } finally {
  ...
  const openDetail = async (row) => {
    ...
    try {
      const data = await apiFetch(`/pos/inventory/categories/${row.id}`, {}, authFetch);
  ...
  const openEdit = async (row) => {
    try {
      ...
      setEditRow({
        id: row.id,
        outlet_id: String(data.outlet_id || row.outlet_id || ""),
  ...
  const saveEdit = async () => {
    ...
    try {
      await apiFetch(
        `/pos/inventory/categories/${editRow.id}`,
        {
          method: "PUT",
          body: JSON.stringify({
            ...
            product_ids: editRow.product_ids.map(Number),
          }),
        },
        authFetch
      );
      ...
      if (expandedId === editedId) {
        const data = await apiFetch(`/pos/inventory/categories/${editedId}`, {}, authFetch);
        setDetail(data);
      }
    } catch (e) {
      setError(e.message);
  ...
  const confirmDelete = async () => {
    ...
    setDeleting(true);
    try {
      await apiFetch(`/pos/inventory/categories/${deleteRow.id}`, { method: "DELETE" }, authFetch);
      setDeleteRow(null);
  ...
  const editStoreProducts = products.filter(
    (p) => !editRow?.outlet_id || String(p.outlet_id) === String(editRow.outlet_id)
  );
  ...
  return (
    <div className="wh-page">
      <PageHeader
        title="Categories"
        description="Manage product categories per store and assign products."
        ...
        {expandedId && detail && (
          <div className="wh-inv-expand-panel">
            ...
            <div className="wh-inv-expand-grid">
              ...
              <div>
                <p className="wh-inv-expand-panel__subtitle">Other products at this store</p>
                <ul className="wh-list">
                  {(detail.all_products || []).filter((p) => p.category_id !== detail.id && String(p.outlet_id) === String(detail.outlet_id)).map((p) => (
      ...
      {editRow && (
        <Modal open title="Edit category" onClose={() => setEditRow(null)} wide className="wh-modal--category">
          <FormField id="edit_cat_outlet" label="Store" as="select" value={editRow.outlet_id} onChange={(e) => setEditRow((r) => ({ ...r, outlet_id: e.target.value, product_ids: [] }))}>
