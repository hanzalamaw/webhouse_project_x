...
export const inventoryController = {
  ...
  async removeWarehouse(req, res) {
    try {
      const id = tryParseEntityId(req.params.id);
      if (!id) return res.status(400).json({ message: "Invalid warehouse id" });
      const result = await inventoryService.removeWarehouse(req.tenantId, id);
      if (!result?.ok) return res.status(404).json({ message: "Warehouse not found" });
      res.json({ success: true, message: result.message, shopifySync: result.shopifySync });
    } catch (e) {
      res.status(e.status || 400).json({ message: e.message });
    }
  },
